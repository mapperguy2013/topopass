...
function createDraft(): MapClickQuestionData {
  return { id: `map-target-${Date.now()}`, type: "map-click", prompt: "", answer: { lat: 51.52, lng: -0.12 }, toleranceMeters: 120, difficulty: "medium", category: "Location knowledge", sourceNote: "Admin browser draft", targetName: "", isActive: false, explanation: "", tip: "", acceptedAreaDescription: "" };
}
        ...
        <section className="space-y-5">
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="space-y-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <label className="block text-sm font-semibold">Accepted area description<textarea className={`${inputClass} min-h-20`} onChange={(event) => setDraft({ ...draft, acceptedAreaDescription: event.target.value })} value={draft.acceptedAreaDescription ?? ""} /></label>
